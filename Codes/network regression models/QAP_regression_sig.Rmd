...
> Predicting the Indonesia coefficient network (whether there's a correlation coefficients >= 0.5 between indicators) from the Indicator text network (whether indicator description similarity >= 0.2) with OLS network model shows that they are correlated at sqrt(0.0001537, which is the R-squared)=0.01239758, with p > 0.05, not statistically significant. 
...
> Predicting the Guatemala coefficient network (whether there's a correlation coefficients >= 0.5 between indicators) from the Indicator text network (whether indicator description similarity >= 0.2) with OLS network model shows that they are correlated at sqrt(0.00002514, which is the R-squared)=0.00501398, with p > 0.05, not statistically significant.  ( useless )
...
> Predicting the Indonesia coefficient network (whether there's a correlation coefficients >= 0.5 between indicators) from the Guatemala coefficient network (whether there's a correlation coefficients >= 0.5 between indicators) with OLS Network model shows that they are correlated at sqrt(0.3356, which is the R-squared)=0.5793099, with p < 0.05 (very close to 0), statistically significant. Such small p value means that we can reject the null hypothesis that there's no releationship between the coefficient network of Indonesia and Guatemala, which means there's nearly no chance for us to see results like this just by accident.
